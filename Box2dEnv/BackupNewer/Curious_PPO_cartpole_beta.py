    ...
    while episode_i < episodes:  # START MAIN LOOP
        ...
        while i_in_batch < N_STEPS:  # START EPISODE BATCH LOOP
            ...
            while not done:  # RUN SINGLE EPISODE
                ...
                with torch.no_grad():
                    alpha, beta = ac_net_actor(torch_state)
                    # val_out = ac_net_critic(torch_state)
                    # curious_out = ac_net_c_critic(torch_state)
                distribution = Beta(alpha[0], beta[0])
                action_beta = distribution.sample()
                clamped_action = (action_beta * 2) - 1
                # if episode_i < 15:
                ...
                action_q.append(action_beta.data.numpy())
                action_log_prob_q.append(distribution.log_prob(action_beta))
        ...
        for epoch in range(K_epochs):
            ...
            while batch_end < batch_length:
                ...
                batch_distribution = Beta(mu_batch, sd_batch)
