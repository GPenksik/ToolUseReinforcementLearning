    ...
    while episode_i < episodes:  ## START MAIN LOOP
        ...
        for epoch in range(K_epochs):
            ...
            while batch_end < batch_length:
                ...
                L_clip = -torch.sum(r_theta_surrogate_min) / r_theta_surrogate_min.size()[
                    0]  # - 0.00001*torch.mean(batch_distribution.entropy())
        ...
        if episode_i % return_time == 0:
            print("%4d, %6.0d, %6.2f, %6.2f, %6.2f, %6.3f" % (
            episode_i, total_i, np.mean(avg_reward_batch), np.mean(avg_reward), np.mean(avg_value_STD),
            np.mean(avg_STD)))
        ## END UPDATING ACTOR
