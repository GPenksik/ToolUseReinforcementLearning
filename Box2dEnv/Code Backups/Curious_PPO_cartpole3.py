...
ac_net_pred = Net_rnd.Net(1, int(N_HIDDEN_RND))
...
def train(episodes):
    ...
    while episode_i < episodes:  # START MAIN LOOP
        ...
        curious_state_t2 = torch.normal(torch.tensor(np.zeros(10000,)), 0.1).unsqueeze(1).float()
        ...
        for epoch in range(R_epochs):
            ...
            while batch_end < batch_length:
                ...
                nn.utils.clip_grad_norm(ac_net_pred.parameters(), 100)
        ...
        if first_batch:
            test_state2 = torch.arange(0, 5, 0.2).unsqueeze(1).float()
        ...
        test_rewards = test_rnd_val # torch.abs(test_rnd_val) #  - test_rnd_val)  # / test_rewards_norm
