...
ac_net_rnd = Net_rnd.Net(1, N_HIDDEN_RND)
ac_net_pred = Net_rnd.Net(1, N_HIDDEN_RND*2)
    ...
    while episode_i < episodes:  # START MAIN LOOP
        ...
        while i_in_batch < N_STEPS:  # START EPISODE BATCH LOOP
            ...
            while not done:  # RUN SINGLE EPISODE
                ...
                curious_state = torch.mul(torch_state[:, 0]+torch.tensor(0.5), 5).unsqueeze(0)
                with torch.no_grad():
                    mu, sd = ac_net_actor(torch_state)
                    val_out, curious_out = ac_net_critic(torch_state)
                    rnd_val = ac_net_rnd(curious_state)
                    pred_val = ac_net_pred(curious_state)
        ...
        curious_state_t = torch.mul(torch.add(torch.rand(10000), 0), 5).unsqueeze(1).float()
        # curious_state_t = torch.normal(torch.tensor(np.zeros(10000,)), 1).unsqueeze(1).float()
        # curious_state_t = torch.add(torch.normal(torch.tensor(np.ones(10000,)), 1), 1).unsqueeze(1).float()
        ...
        for epoch in range(R_epochs):
            ...
            while batch_end < batch_length:
                ...
                nn.utils.clip_grad_norm(ac_net_pred.parameters(), 10)
                # nn.utils.clip_grad_value_(ac_net_pred.parameters(), 100)
                # clip_min_grad_value_(ac_net_pred.parameters(), 1)
                optimizer_rnd.step()
        ...
        test_state = torch.arange(-5, 5, 0.99).unsqueeze(1).float()
        with torch.no_grad():
