    ...
    while episode_i < episodes:  # START MAIN LOOP
        ...
        while episode_in_batch < N_TRAJECTORIES:
            ...
            while not done:  # RUN SINGLE EPISODE
                ...
                if i_in_episode % 1 == 0 and episode_i % 200 == 0 and episode_i >= 0:
                    env.render()
        ...
        if episode_i % return_time == 0:
            print("%4d, %6.0d, %6.2f, %6.2f"
                  % (episode_i, total_i, np.mean(avg_reward_batch), np.mean(avg_reward)))

        with open('C:\\Users\\genia\\source\\repos\\Box2dEnv\\Box2dEnv\\saves\\{}.csv'.format(nName), 'a+') as csv:
            for ret_write in zip(np.asarray(avg_reward)):
                csv.write("{:2.2f}\n".format(ret_write[0]))
