...
def main():
    ...
    load_agent = False
    agent_filename = '861-P33-20-PPO-2000'
    to_visualize = False
    ...
    environment2.gym.seed(random_seed+1)
    environment2.gym.unwrapped.set_reward(2)
    environment2.gym.unwrapped.set_random(2)

    # Initialize Agent-Network-Model objects

    with open('C:\\Users\\genia\\Source\\Repos\\Box2dEnv\\examples\\configs\\900-ppo.json', 'r') as fp:
        agentSpec = json.load(fp=fp)
    with open('C:\\Users\\genia\\Source\\Repos\\Box2dEnv\\examples\\configs\\Test-ppo.json', 'r') as fp:
        agentSpec2 = json.load(fp=fp)
    with open('C:\\Users\\genia\\Source\\Repos\\Box2dEnv\\examples\\configs\\900-mlp2_network.json', 'r') as fp:
        network = json.load(fp=fp)

    # agentSpec['update_mode'].update(batch_size=3)
    # agentSpec['update_mode'].update(frequency=3)
    ...
    nName.append(("{}-{}{}{}-{}-{}".format(str(run_number+1).zfill(3), nTask, nReward, nRandom, nSeed, nAlg)))
    ...
    def episode_finished(r, id_):
        ...
        save_period = 5
        reward_tracker.append(id_)
        if r.episode % visualize_period == 0:
            if to_visualize:
                environment[id_].visualize = True  # Set to true to visualize
        else:
            environment[id_].visualize = False

        if r.episode % save_period == 0:
            with open('C:\\Users\\genia\\Source\\Repos\\Box2dEnv\\Box2dEnv\\saves\\{}.csv'.format(nName[id_]), 'a+') as csv:
        ...
        if r.episode % report_episodes == 0:
            ...
            logger.info("{:6d},    {:+6.2f},     {:+6.2f}".format(r.agent[id_].episode, r.episode_rewards[-1],
        ...
        if r.episode == 1 or (r.episode % agent_save_period == 0):
            logger.info(
                "Saving agent to {} at episode {}".format(
                    'C:\\Users\\genia\\Source\\Repos\\Box2dEnv\\Box2dEnv\\saves\\{}'.format(nName[id_]), r.episode))
            r.agent[id_].save_model(directory='C:\\Users\\genia\\Source\\Repos\\Box2dEnv\\Box2dEnv\\saves\\modelSave\\{}-{}'.format(nName[id_], r.episode),
