...
def main():
    ...
    with open('C:\\Users\\genia\\Source\\Repos\\Box2dEnv\\examples\\configs\\{}'.format(args.agent), 'r') as fp:
    ...
    with open('C:\\Users\\genia\\Source\\Repos\\Box2dEnv\\examples\\configs\\{}'.format(args.network), 'r') as fp:
        network = json.load(fp=fp)
    ...
    def episode_finished(r, id_):
        ...
        if r.episode % visualize_period == 0:
            if to_visualize:
                environment[0].visualize = True  # Set to true to visualize
        else:
            environment[0].visualize = False
        ...
        if r.episode % report_episodes == 0:
            ...
            logger.info("{:6d},    {:+6.2f},     {:+6.2f}".format(r.agent[0].episode, r.episode_rewards[-1],
        ...
        if r.episode == 1 or (r.episode % agent_save_period == 0):
            ...
            r.agent[0].save_model(directory='C:\\Users\\genia\\Source\\Repos\\Box2dEnv\\Box2dEnv\\saves\\modelSave\\{}-{}'.format(nName, r.episode),
    ...
    logger.info("Learning finished. Total episodes: {ep}".format(ep=1))
