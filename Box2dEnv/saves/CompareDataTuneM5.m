...
RL = [1 2 3 1 2 3 1 2 3] ; RndL = [1 1 1 2 2 2 3 3 3]; Tsk = ["P" "L"];
runNumbers = [4108 4107 4106 4105 4104 4103 4102 4101 4100];
...
for m = 1:length(Tsk)
    ...
    for n = 1:length(runNumbers)
        number = num2str(runNumbers(n),'%04.f');%(num,'%03.f')
        %for u = 1:nParams
        RLx = num2str(RL(n));
        RndLx = num2str(RndL(n));
...
for j=1:length(Tsk)
    for i=1:length(runNumbers)
        for k=1:length(seeds)
            if names(j,i,k) == "" || ismissing(names(j,i,k))
...
%     ylabel("Moving average of reward")
    ...
    box on
    RLx = num2str(RL(i));
    RndLx = num2str(RndL(i));
    title("Reward = " + RLx + ", Random = " +RndLx);
end
%legend
%%
for j = 1
    for i = 1:nRuns
        for k = 1:nSeeds
            maxMean(1,i,k) = max(meanData{j,i,k});
            maxMeanCompleted(1,i,k) = max(meanCompleted{j,i,k});
            maxMeanOverSeeds(1,i) = mean(maxMean(1,i,:));
            maxMeanCompletedOverSeeds(1,i) = mean(maxMeanCompleted(1,i,:));
            maxVarOverSeeds(1,i) = std(maxMean(1,i,:))^2;
        end
    end
end
maxMeanOverSeedsRe = reshape(maxMeanOverSeeds,[3 3])'
maxVarOverSeedsRe = reshape(maxVarOverSeeds,[3 3])'
maxMeanCompletedOverSeedsRe = reshape(maxMeanCompletedOverSeeds,[3 3])'
