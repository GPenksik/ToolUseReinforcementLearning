...
names = strings(length(Tsk), nParams, nSeeds);
for m = 1:length(Tsk)
    Tskx = Tsk(m);
    for n = 1:nParams
        ...
        %for x = 1:nParams
            for u=1:length(seeds)
                % for x = 1:length(RL)
                    x = n;
                    RLx = num2str(RL(x));
                    RndLx = num2str(RndL(x));
                    number = num2str(runNumbers(u),'%04.f');%(num,'%03.f')
...
Data = cell([length(Tsk),nParams,nSeeds]);
meanData  = cell([length(Tsk),nParams,nSeeds]);
meanCompleted  = cell([length(Tsk),nParams,nSeeds]);
completed = cell([length(Tsk),nParams,nSeeds]);
